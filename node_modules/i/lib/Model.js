...
Model.prototype.retrieveHash = function(key, cb) {
    ...
    db.hvals(this.abb + key, function(err, data) {
        if (err) throw new I.Exception(10103, err);
...
Model.prototype.retrieveList = function(key, cb) {
    ...
    db.hkeys(this.abb + key, function(err, data) {
        if (err) throw new I.Exception(10107, err);
        // empty list
        if (data.length === 0) {
...
Model.prototype.add = function(object, cb) {
    if (this.type === I.Const.OBJECT_TYPE_LIST) throw new I.Exception(10108);
    var self = this;
    if (this.pkAutoIncrement) {
        db.incr(I.Const.GLOBAL_KEY_PREFIX + this.abb, function(err, id) {
            if (err) throw new I.Exception(10104, err);
...
Model.prototype.delHash = function(object, cb) {
    db.del(this.abb + object.getPK(), function(err, result) {
        if (err) throw new I.Exception(10110, err);
        cb(object);
    });
};
...
Model.prototype.delList = function(list, cb) {
    ...
    dbMulti.exec(function(err, results) {
        if (err) throw new I.Exception(10116, err);
        list.reset();
        cb(list);
    });
};
...
Model.prototype.updateList = function(list, cb) {
    ...
    var exec = function exec() {
        dbMulti.exec(function(err, results) {
            if (err) throw new I.Exception(10117, err);
            cb(list);
        });
    };
